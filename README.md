...
| [`rc_tabm_variants/`](rc_tabm_variants) | Supplementary Information | Mean-TabM and Standalone CondPool-TabM comparison variants referenced alongside RC-TabM. |
| [`third_party/tabm/`](third_party/tabm) | — | Vendored TabM implementation that `residual_predictor/` and `rc_tabm_variants/` depend on. |
...
identifiers used throughout `pamp_search/` and `cost_benefit_analysis/` differ. This is
