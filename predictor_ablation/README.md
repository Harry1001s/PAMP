...
- `predictor_ablation/` — package: `models.py` (configurable two-modality predictor
  architectures, incl. the `ConcatPredictor` fusion variants), `config.py`, `data.py`,
  `preprocessing.py`, `training.py`, `metrics.py`, `parameter_budget.py`, `registry.py`,
  `reproducibility.py`.
...
experiment directory) is not part of this repository. See `ORIGINAL_README.md` for the
original experiment-directory notes.
