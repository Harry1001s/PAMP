...
|---|---|---|
| **Data partitions** — the 80:10:10 train/val/test split (22,126 / 2,766 / 2,766 records) over the CataPro-derived 27,658-record enzyme–substrate dataset | `original_predictor/`, `residual_predictor/`, `extra_trees_baseline/` | Not released here |
| **Design-cohort membership** — the 2,754-pair filtered test cohort (2,391 unique enzymes, 103–2,432 residues) used for mutation design | `pamp_search/` | Not released here |
| **Candidate-level predictions** — per-record, per-method proposed substitutions and predicted `Δlog2(kcat)` (Tables II/III) | `pamp_search/build_paper_results.py` and related aggregation scripts | Not released here |
| **Algorithm settings** — frozen hyperparameters/config locked in for the reported runs (e.g. `residual_predictor/config.json`, `extra_trees_baseline`'s selected parameters) | training/search scripts throughout | Partially included where small (e.g. `residual_predictor/config.json`); larger per-run manifests not included |
